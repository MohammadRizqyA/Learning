...
$storeID = mysqli_real_escape_string($conn, $storeID);

$storeQuery = "SELECT storeName, storeImage FROM store WHERE storeID = '$storeID'";
$storeResult = mysqli_query($conn, $storeQuery);
$store = mysqli_fetch_assoc($storeResult);

$productQuery = "SELECT  p.productID, p.productName, p.productDescription, p.price, p.stock, p.productImage, s.storeName, s.storeImage
                 FROM product p
